    ...
    <PhoneFrame label="Customer scans your QR code">
      <div className="px-4 pt-2">
        ...
        <div className="grid grid-cols-3 gap-1">
          {[10,20,50,100,200].map((a,i)=>{const sel=i===2;return(<div key={a} className="py-1.5 text-center text-[9px] font-bold rounded" style={{ background: sel?"rgba(20,167,249,0.15)":"rgba(255,255,255,0.04)", color: sel?"#14a7f9":"rgba(255,255,255,0.35)", border: sel?"1px solid rgba(20,167,249,0.3)":"1px solid rgba(255,255,255,0.06)" }}>R{a}</div>);})}
          <div className="py-1.5 text-center text-[9px] rounded" style={{ background:"rgba(255,255,255,0.04)", color:"rgba(255,255,255,0.2)", border:"1px solid rgba(255,255,255,0.06)" }}>Custom</div>
        </div>
...
const stepData = [
  ...
  { num: "03", title: "Accept Payments", items: ["When a customer wants to tip you, they scan your QR code with their phone camera", "They don't need to download any app — it opens directly in their browser", "They pick an amount (R10, R20, R50, R100, R200) or enter a custom amount", "They pay securely with a bank card via Paystack (3D Secure)", "You get a notification and the tip appears in your digital wallet instantly"] },
  ...
  { num: "06", title: "Withdraw Your Tips", items: ["Go to Dashboard → Withdraw", "Choose your withdrawal method: Instant Money (collect at any ATM) or EFT (bank account)", "Minimum withdrawal is R100", "A 10% platform fee is deducted on withdrawal — this is the only fee", "Funds have a 72-hour cooldown before they become available"] },
...
const faqs = [
  { q: "Do I need to pay anything to sign up?", a: "No. Creating an account and receiving tips is completely free. A 10% platform fee is only deducted when you withdraw." },
